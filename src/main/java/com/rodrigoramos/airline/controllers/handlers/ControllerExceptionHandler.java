package com.rodrigoramos.airline.controllers.handlers;
...
public class ControllerExceptionHandler {
    ...
    public ResponseEntity<CustomError> noAvailableSeats(NoAvailableSeatsException e, HttpServletRequest request) {
    ...
    public ResponseEntity<CustomError> duplicateTicket(DuplicateTicketException e, HttpServletRequest request) {
        HttpStatus status = HttpStatus.CONFLICT;
        CustomError err = new CustomError(Instant.now(), status.value(), e.getMessage(), request.getRequestURI());
        return ResponseEntity.status(status).body(err);
    }
